...
public class Permission {
    ...
    public static void checkIn(int day){
        if(day>=2 && day <=6 ) System.out.println("if it is weekday user is allowed to check in from 10 AM to 5 PM");
        else if (day == 1 || day == 7) System.out.println("if it is weekend user is allowed to check in 10 AM to 3 PM");
        else throw new RuntimeException("The input does not represent any day!!!");
    }
    public static void checkedIn(int day){
        switch (day){
            case 1:
            case 7:
